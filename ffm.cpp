...
int main()
{
    int blockNo,processNo,blockSize[10],processSize[10],flags[10],alloc[10],temp,highest=-9999,id=9,i,j;
    ...
    for(i = 0; i < blockNo; i++){
        cout << "\n" << i+1 << "\t\t" << blockSize[i] << "\t\t";
        if(flags[i] == 1)
            cout << alloc[i]+1 << " \t\t" << processSize[alloc[i]] <<  "\t\t" << blockSize[i]-processSize[alloc[i]];
        else
